...
# filter the actual errors from the log (ignore autogenerated stuff from protocol buffers for now)
if grep -v "/G__" error-log.txt | grep -v ".pb.cc" | grep " error:" ; then
  exit 1
fi
