...
prefer_system: (?!slc5)
prefer_system_check:
  python -c 'import sys; import sqlite3; sys.exit(1 if sys.version_info < (2, 7) else 0)' && pip --help > /dev/null && printf '#include \"pyconfig.h"' | gcc -c -I$(python-config --cflags) -xc -o /dev/null -
---
#!/bin/bash -ex
