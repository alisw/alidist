...
if [[ -d $SOURCEDIR/interpreter/llvm ]]; then
  ...
  ROOT_HAS_PYTHON=1
  # One can explicitly pick a Python version with -DPYTHON_EXECUTABLE=... -DPYTHON_INCLUDE_DIRS=<path_to_Python.h>
  PYTHON_EXECUTABLE=$(python3-config --exec-prefix)/bin/python3
  PYTHON_INCLUDE_DIRS=$(python3-config --includes | sed -e's/^[ ]*-I//' | cut -f1 -d' ')
