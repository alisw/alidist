...
prefer_system_check: |
  ...
  while true; do
    ...
    # - 1 : detect and fail if a backend is not found
  ...
  EOF2
    ...
    fi # default ALIBUILD_O2_FORCE_GPU

    if [[ ${ALIBUILD_O2_FORCE_GPU} == "1" ]] && [[ $GPU_HIP_ENABLED != 1 || $GPU_CUDA_ENABLED != 1 || $GPU_OPENCL_ENABLED != 1 ]]; then
      # We abort with a version "error", which will fail below
      echo ALIBUILD_O2_FORCE_GPU=1 set, but not all GPU backends detected 1>&2
      GPU_FEATURES=error
      break
    fi
...
prefer_system_replacement_specs:
  ".*":
    version: "%(key)s"
    recipe: |
      if [[ "${PKG_VERSION}" == "error" ]]; then
        echo gpu-system.sh GPU detection failed
        exit 1
      fi
      mkdir -p "$INSTALLROOT"/etc
      rm -f "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_ROCM_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)rocm(-|_|$) ]] && echo 1 ) || ( [[ "${PKG_VERSION}" =~ (^|-)auto(-|_|$) ]] && echo auto || echo 0 ) )\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_CUDA_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)cuda(-|_|$) ]] && echo 1 ) || ( [[ "${PKG_VERSION}" =~ (^|-)auto(-|_|$) ]] && echo auto || echo 0 ) )\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_OPENCL_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)opencl(-|_|$) ]] && echo 1 ) || ( [[ "${PKG_VERSION}" =~ (^|-)auto(-|_|$) ]] && echo auto || echo 0 ) )\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_MIOPEN_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)miopen(-|_|$) ]] && echo 1 ) || echo 0)\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_CUDNN_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)cudnn(-|_|$) ]] && echo 1 ) || echo 0)\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_MIGRAPHX_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)migraphx(-|_|$) ]] && echo 1 ) || echo 0)\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh
      echo "export O2_GPU_TENSORRT_AVAILABLE=\"$( ( [[ "${PKG_VERSION}" =~ (^|-)tensorrt(-|_|$) ]] && echo 1 ) || echo 0)\"" >> "$INSTALLROOT"/etc/gpu-features-available.sh#
---
