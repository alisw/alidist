...
if [ -n $ALIROOT_ROOT ] && [ -e $ALIROOT_ROOT/lib/libO2TPCCATracking.so ]; then
    HLT_O2_TPCCA_TRACKING_LIB_ROOT=$ALIROOT_ROOT
    DEPENDENCY_HLTO2TPCCA=
else
    HLT_O2_TPCCA_TRACKING_LIB_ROOT=$HLTO2CATRACKING_ROOT
    DEPENDENCY_HLTO2TPCCA=HLTO2CATracking/$HLTO2CATRACKING_VERSION-$HLTO2CATRACKING_REVISION
fi
...
cmake $SOURCEDIR -DCMAKE_INSTALL_PREFIX=$INSTALLROOT                                              \
      ...
      -DO2_TPCCA_TRACKING_LIB_DIR=$HLT_O2_TPCCA_TRACKING_LIB_ROOT
...
# Dependencies
module load BASE/1.0 FairRoot/$FAIRROOT_VERSION-$FAIRROOT_REVISION ${DDS_VERSION:+DDS/$DDS_VERSION-$DDS_REVISION} ${GCC_TOOLCHAIN_VERSION:+GCC-Toolchain/$GCC_TOOLCHAIN_VERSION-$GCC_TOOLCHAIN_REVISION} ${VC_VERSION:+Vc/$VC_VERSION-$VC_REVISION} $DEPENDENCY_HLTO2TPCCA
# Our environment
